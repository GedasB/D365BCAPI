...
class Connect(object):
    ...
    def __init__(self, url, auth=None, headers=None):
        ...
        self.filter_text = str()  # filter_text can be modified by call object.filter_text =
    ...
    def insert(self, jsonbody):
        """
        creates record specified in jsonbody. primary key fields values must be filled in. Except when it is created
        ...
        endpoint url can be changed before call
        record id is not required
        :param: jsonbody: dictionary(json) with primary key fields and values (mandatory)
                and other fields and values
        :return: list [response status code (201), response message (Created)];
        """

        try:
            response = requests.post(self.url, auth=self._auth, headers=self._headers, json=jsonbody)
        except requests.exceptions.ConnectionError as ex_err:
            self.except_error = ex_err
            return []
    ...
    def modify(self, jsonbody):
        """
        modify record specified by id with values specified in jsonbody
        endpoint url can be changed before call
        record id in endpoint url is required for execution
        :param jsonbody: json with record fields and values need to be modified
        :return: list [response status code (200), response message (OK)]
        """
        ...
        response = requests.patch(self.url, auth=self._auth, headers=self._headers, json=jsonbody)
    ...
    def delete(self):
        ...
        endpoint url can be changed before call
        record id in endpoint url is required for execution
        :return: list [response status code (204 - OK), response message (No Content - OK)]
